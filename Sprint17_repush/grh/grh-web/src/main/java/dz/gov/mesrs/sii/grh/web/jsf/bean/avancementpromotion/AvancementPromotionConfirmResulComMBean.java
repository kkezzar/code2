...
@ManagedBean(name = "avancementPromotionConfirmResulComMBean")
@ViewScoped
/**
 * 
 * @author SLIMANE Ramdane  on : 04 dec. 2014  14:51:27
 */
public class AvancementPromotionConfirmResulComMBean  extends BaseBean{
	...
	private String dureeProposablePromotion,typePromotionSelected;
	...
	public AvancementPromotionConfirmResulComMBean() {
	...
	private void loaddossierEmployeModel() {
		dossierEmployeModel = new LazyDataModel<DossierEmployeDto>() {
			...
			public List<DossierEmployeDto> load(int first, int pageSize, String sortField, SortOrder sortOrder,
					...
					List<Long> listDossierEmployeID= serviceLocator.getDossierEmployeService().findListePromotionParModalConfID(paramettres,listNomenclatureActifDetachementID,listnomenclatureBytypePromotionModaliteID ,listNomenclatureModaliteRecrutementID);
	...
	public void onRowSelect(SelectEvent event) {
		...
		if (dossierEmployeDto == null) {employeProposeDto = new EmployeProposeDto();}else{
		    List<CarriereDto> carrieres = dossierEmployeDto.getCarriereDtos();
		    if(carrieres!=null)if(!carrieres.isEmpty()){
			for(CarriereDto car:carrieres){
				if(car!= null){
				if(car.getNomenclatureBytypePromotionDto()!=null)	if((car.getConfirm()== null)
						&&( (car.getNomenclatureBytypePromotionDto().getId()== 648753)||(car.getNomenclatureBytypePromotionDto().getId()== 648754)
								||(car.getNomenclatureBytypePromotionDto().getId()== 648755))){
					carriereDto=car;
				}	
				}
			}
		...
		if(carriereDto==null)carriereDto= new CarriereDto();
	}
	...
	public void ajouterEmploye() {	
		carriereDto.setConfirm(true);
		carriereDto=this.serviceLocator.getCarriereService().save(carriereDto);
		dossierEmployeDto.setCarriereCouranteDto(carriereDto);
		dossierEmployeDto=this.serviceLocator.getDossierEmployeService().saveDossierEmployeByCarriere(carriereDto); 
		//employeProposeDto=this.serviceLocator.getEmployeProposeService().save(employeProposeDto);
