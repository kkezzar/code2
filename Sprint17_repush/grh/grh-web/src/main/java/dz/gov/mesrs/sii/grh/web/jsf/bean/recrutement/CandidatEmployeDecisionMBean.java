...
@ManagedBean(name = "candidatEmployeDecisionMBean")
@ViewScoped
/**
 * 
 * @author BELDI Jamel  on : 16 nov. 2014  14:51:27
 */
public class CandidatEmployeDecisionMBean extends BaseBean {
	...
	public CandidatEmployeDecisionMBean() {
	...
	private boolean validateForm() {
		...
		boolean result = true;
		if (candidatEmployeDto.getDateDecesion() != null
				&& candidatEmployeDto.getDateDecesion().before(
						candidatEmployeDto.getDateSelection())) {
			GRHMessagesUtils
					.showErrorMessage("date_decision_candidat_invalide");
			result = false;

		}
		if (candidatEmployeDto.getAdmis() != null
				&& candidatEmployeDto.getAdmis() == true) {
			candidatEmployeDto.setMotifConvocation(null);
			if (candidatEmployeDto.getDateConvocation() != null
					&& candidatEmployeDto.getDateConvocationRec().before(
							candidatEmployeDto.getDateDecesion())) {
				GRHMessagesUtils
						.showErrorMessage("date_convocation_decision_candidat_invalide");
				result = false;
			}
		} else {
			candidatEmployeDto.setDateConvocationRec(null);
